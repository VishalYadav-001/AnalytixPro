...
async function uploadDataset() {
  ...
  success.value = ""
  try {
    const res = await datasetService.upload({ file: selectedFile.value, name: selectedFile.value.name })
    const datasetId = res.data?.id
    selectedFile.value = null

    // Auto-create a chat session and navigate there immediately
    if (datasetId) {
      const sessionRes = await chatService.create({ dataset: datasetId })
      chatSessions.value.push(sessionRes.data)
      router.push(`/chat/${sessionRes.data.id}`)
      return
    }
