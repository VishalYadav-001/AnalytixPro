    ...
    <div v-if="!loading && dashboards.length" class="grid md:grid-cols-2 gap-5">
      ...
      >
        <div class="flex items-start justify-between gap-3 mb-4">
          <div class="min-w-0">
            <h2
              class="font-semibold text-slate-900 truncate group-hover:text-brand-600 transition-colors cursor-pointer"
              @click="viewDashboard(db)"
            >
              {{ db.title }}
            </h2>
            <p class="text-xs text-slate-400 mt-0.5">{{ db.dataset_name }} · {{ formatDate(db.created_at) }}</p>
          </div>
        ...
        <div class="flex flex-wrap gap-2">
          ...
          <div class="relative" ref="exportMenuRef">
            <button class="btn-secondary btn-sm" @click="toggleExportMenu(db.id)">
              Export
