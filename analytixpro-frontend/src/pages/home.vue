<template>
  <div class="space-y-6 md:space-y-8 animate-fade-up px-1">
    <div>
      <h1 class="page-title text-xl md:text-2xl">
        Welcome back, {{ displayName }}
      </h1>
      <p class="page-subtitle">
        Here's a snapshot of your analytics workspace.
      </p>
    </div>
    ...
    <div>
      <h2 class="font-display font-semibold text-lg text-slate-900 mb-4">
        Quick Actions
      </h2>
    ...
    <div v-if="recentDatasets.length">
      <div class="flex items-center justify-between mb-4">
        <h2 class="font-display font-semibold text-lg text-slate-900">
          Recent Datasets
        </h2>
        <RouterLink
          to="/datasets"
        ...
        <div class="overflow-x-auto w-full">
          <table class="data-table whitespace-nowrap md:whitespace-normal">
            ...
            <tbody>
              ...
              >
                <td
                  class="font-medium text-slate-800 truncate max-w-[150px] md:max-w-none"
                >
                  {{ ds.name }}
                </td>
                <td class="text-slate-500 uppercase text-xs">
                  {{ ds.file_type }}
                </td>
      ...
      h(
        ...
        props.to
          ? {
              to: props.to,
              class:
                "card p-4 md:p-5 flex items-center gap-4 hover:shadow-md transition-shadow cursor-pointer",
            }
        ...
        [
          h(
            "div",
            {
              class: `w-12 h-12 rounded-xl ${c.bg} flex items-center justify-center shrink-0`,
            },
            [
              h("svg", {
                viewBox: "0 0 24 24",
          ...
          h("div", { class: "truncate" }, [
            h(
              ...
              {
                class:
                  "text-xl md:text-2xl font-display font-bold text-slate-900",
              },
              props.value
            ),
            h(
  ...
  setup(props) {
    ...
    return () =>
      h(
        ...
        {
          class:
            "card p-5 md:p-6 flex flex-col gap-4 hover:shadow-md transition-shadow h-full",
        },
        [
          h(
            "div",
            {
              class:
                "w-10 h-10 rounded-xl bg-brand-50 flex items-center justify-center",
            },
            [
              h("svg", {
                viewBox: "0 0 24 24",
          ...
          h(
            RouterLink,
            { to: props.to, class: "btn-primary btn-sm self-start mt-auto" },
            () => props.cta + " →"
          ),
        ]
      );
...
const StatusBadge = {
  ...
  setup(props) {
    ...
    return () =>
      h(
        "span",
        { class: `badge ${cls[props.status] ?? "badge-gray"}` },
        props.status
      );
  },
};
