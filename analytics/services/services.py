...
_FIELD_PARSERS = {
    'analysis_type': lambda v: (
        ...
        n if n in ('sales', 'hr', 'financial', 'custom') else None
    )[-1],
    'goal': lambda v: (
        n := v.strip().lower().replace(' ', '_'),
        n if n in ('find_trends', 'predict_outcomes', 'custom') else None
    )[-1],
    'target_column':   lambda v: TARGET_COLUMN_NONE_SENTINEL if v.strip().lower() == 'none' else v.strip(),
    'dashboard_level': lambda v: (
        n := v.strip().lower(),
        n if n in ('basic', 'advanced') else None
    )[-1],
...
def run_eda_analysis(dataset, analysis_type: str = 'eda',
    ...
    try:
        ...
        if chat_session_id:
            try:
                chat_session = ChatSession.objects.get(id=chat_session_id, user=dataset.user)
...
def _compute_categorical_insights(df: pd.DataFrame) -> dict:
    ...
    insights = {}
    for col in cat_cols:
        insights[col] = df[col].value_counts().head(10).to_dict()
...
def _export_notebook(dashboard: Dashboard):
    ...
    cells = [
        ...
        _nb_code_cell(
            f'corr = {json.dumps(analysis.correlation_matrix, indent=2)}\n'
...
def _is_field_answered(session: ChatSession, field: str) -> bool:
    if field == 'download_code':
        return session.download_code is not None
        ...
        if current_field == 'download_code':
            ...
            for field, question in _CHAT_FLOW:
                if field == current_field:
                    return f"That doesn't look right. Please choose one of the options:\n\n{question}", session
