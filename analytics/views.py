...
class DatasetViewSet(viewsets.ModelViewSet):
    ...
    def run_analysis(self, request, pk=None):
        ...
        if chat_session_id:
            existing = Analysis.objects.filter(chat_session_id=chat_session_id).first()
            if existing:
                return Response(
...
class DashboardViewSet(viewsets.ModelViewSet):
    ...
    def generate(self, request):
        ...
        analysis = get_object_or_404(Analysis, id=analysis_id, dataset__user=request.user)

        if hasattr(analysis, 'dashboard'):
            return Response(
                DashboardDetailSerializer(analysis.dashboard, context={'request': request}).data,
                status=status.HTTP_200_OK,
            )
    ...
    def export(self, request, pk=None):
        ...
        try:
            report = export_dashboard_report(dashboard=dashboard, user=request.user, fmt=fmt)
        except Exception as exc:
